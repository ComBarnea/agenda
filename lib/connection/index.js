 ...
 */
ConnectionDriver.prototype.init = function (type, url, collectionName, dbName, options, db, collection) {
    var self = this;
    self._url = null;
    ...
    if(type === 'mongoDB') {
        ...
        self._type = type;
        self._connection = new sqlConnection();
...
ConnectionDriver.prototype.connect = function (cb) {
    ...
    self._connection.connect(self._url, self._dbName ,self._collectionName, self._options, function (err, db) {
 ...
 * @param cb
 */
ConnectionDriver.prototype.purge = function(names, cb) {
    var self = this;
    self._connection.purge(self._collection, names, function (err, jobs) {
        if(err) return cb(err);
...
ConnectionDriver.prototype.update = function(query, sort, update, options, cb) {
    ...
    self._connection.update(self._collection, query, sort, update, options, function (err, jobs, met) {
        if(err) return cb(err);
...
ConnectionDriver.prototype.insertOne = function(data, cb) {
    var self = this;
    self._connection.insertOne(self._collection, data, function (err, job) {
        if(err) return cb(err);
