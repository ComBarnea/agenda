...
var MongoConnection = module.exports = function () {
    if (!(this instanceof MongoConnection)) return new MongoConnection();
 ...
 * @param cb
 */
MongoConnection.prototype.connect = function (url, db, collection, options, cb) {
...
MongoConnection.prototype.db_init = function (mdb, collectionName, collection, cb) {
    var collection =  mdb.collection(collectionName);
...
MongoConnection.prototype.purge = function(collection, names, cb) {
    var query = {name: {$not: {$in: names}}};
 ...
 * @param cb
 */
MongoConnection.prototype.update = function(collection, query, sort, update, options,  cb) {
    var updateObj = update ? updateBuilder(update.update, update.set, update.setOnInsert) : null;

    collection.findAndModify(query, sort, updateObj, options, function( error, result ) {
        if (cb) {
            cb( error, result && result ? result.value : undefined );
        }
    });
};
 ...
 * @param cb
 */
MongoConnection.prototype.updateMany = function(collection, query, update, cb) {
    var updateObj = update ? updateBuilder(update.update, update.set, update.setOnInsert) : null;

    collection.updateMany(query, updateObj, function( error, result ) {
        if (cb) {
            cb( error, result && result.result ? result.result.n : undefined );
        }
    });
...
MongoConnection.prototype.insertOne = function(collection, data, cb) {
    collection.insertOne(data, function( error, result ){
        if (cb) {
            cb( error, result && result.ops ? result.ops[0] : undefined );
        }
    });
};
...
function updateBuilder(update, set, setOnInsert) {
    var newUpdate = {};
    if(update) newUpdate = update;
    if(set) newUpdate.$set = set;
    if(setOnInsert) newUpdate.$setOnInsert = setOnInsert;
