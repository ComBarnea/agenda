...
var Agenda = module.exports = function(config, cb) {
  ...
  this._jobsToLock = [];
  if (config.mongo) {
    Connection.init('mongoDB', null, null, null, config.mongo, config.db ? config.db.collection : undefined);


    this.db_init(cb);
  } else if (config.db && config.dbType === 'mongoDB') {
    Connection('mongoDB', null, null, null);
...
Agenda.prototype.db_init = function(cb){
  ...
  Connection.db_init(function (err, collection) {
    if(err) self.emit('error', err);
...
Agenda.prototype.jobs = function( query, cb ){
  var self = this;
  Connection.getJobs(query, function (error, result) {
    var jobs;
    if( !error ){
...
Agenda.prototype.cancel = function(query, cb) {
  Connection.cancel( query, function( error, result ){
    if (cb) {
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    Connection.update({_id: id}, {}, update, {new: true}, processDbResult );
    ...
    // Try an upsert.
    Connection.update({name: props.name, type: 'single'}, {}, update, {upsert: true, new: true}, processDbResult);
  } else if (unique) {
    var query = job.attrs.unique;
    query.name = props.name;
    if( uniqueOpts && uniqueOpts.insertOnly )
      update = { $setOnInsert: props };
    Connection.update.findAndModify(query, {}, update, {upsert: true, new: true}, processDbResult);
  } else {
    Connection.insertOne(props, processDbResult);    // NF updated 22/04/2015
  }
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  Connection.findAndLockNextJob(jobName,definition,this._nextScanAt, createJob, this, cb);
};
...
Agenda.prototype._unlockJobs = function(done) {
  ...
  Connection.updateMany({_id: { $in: jobIds } }, { $set: { lockedAt: null } }, done);    // NF refactored .update() 22/04/2015
};
...
function processJobs(extraJob) {
  ...
  function lockOnTheFly() {
    ...
    Connection.update(criteria, sort, update, options, function(err, resp) {
      if (resp.value) {
