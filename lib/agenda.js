...
var Agenda = module.exports = function(config, cb) {
  ...
  this._jobsToLock = [];
  if (config.mongo) {
    ConnectionDriver.init('mongoDB', null, null, null, config.mongo, config.db ? config.db.collection : undefined);


    this.db_init(cb);
  } else if (config.db && config.dbType === 'mongoDB') {
    ConnectionDriver.init('mongoDB', config.db.address, config.db.collection, config.db.options);


    this.connect(cb);
  } else {
    throw new Error('No db type found');
  }
};
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    ...
    ConnectionDriver.update.findAndModify(query, {}, update, {upsert: true, new: true}, processDbResult);
  } else {
