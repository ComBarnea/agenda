 ...
 *  - Renamed _mdb to _collection because it is a collection, not a db.
...
var ConnectionDriver = require('./connection')();
...
var Agenda = module.exports = function(config, cb) {
  ...
  this._onlyMaster = config.onlyMaster ? true : false;
  this._isMaster = config.isMaster ? true : false;
  ...
  // default to mongoDB
  if(config.db && !config.dbType)  config.dbType = 'mongoDB';

  if (config.mongo) {
    this._mdb = config.mongo;
    ConnectionDriver.init('mongoDB', null, config.db ? config.db.collection : undefined, null, null, config.mongo, null);

    this.mongo(config.mongo, config.db ? config.db.collection : undefined, cb);
    this.db_init(cb);
  } else if (config.db && config.dbType === 'mongoDB') {
    ConnectionDriver.init('mongoDB', config.db.address, config.db.collection, null, config.db.options);

    this.connect(cb);
  } else if(config.db && config.dbType === 'sqlDB') {
    ConnectionDriver.init('sqlDB', config.db.address, config.db.collection, config.db.dbName, config.db.options);
 ...
 *  @param cb
 */
Agenda.prototype.connect = function(cb) {
  if(!_runAllowed(this._onlyMaster, this._isMaster)) return;
  var self = this;

  ConnectionDriver.connect(function (err, db) {
 ...
 *  @param cb
 */
Agenda.prototype.db_init = function(cb) {
  if(!_runAllowed(this._onlyMaster, this._isMaster)) return;
  var self = this;

  ConnectionDriver.db_init(function (err, collection) {
...
Agenda.prototype.jobs = function( query, cb ) {
  ...
  var self = this;
  ConnectionDriver.getJobs(query, function (error, result) {
    var jobs;
    if( !error ){
...
Agenda.prototype.purge = function(cb) {
  ...
  ConnectionDriver.purge(definedNames, cb);
...
Agenda.prototype.cancel = function(query, cb) {
  ...
  ConnectionDriver.cancel( query, function( error, result ){
    if (cb) {
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    ConnectionDriver.update({_id: id}, {}, update, {new: true}, processDbResult );
    ...
    // Try an upsert.
    ConnectionDriver.update({name: props.name, type: 'single'}, {}, update, {upsert: true, new: true}, processDbResult);
  } else if (unique) {
    var query = job.attrs.unique;
    query.name = props.name;
    if( uniqueOpts && uniqueOpts.insertOnly )
      update = { setOnInsert: props };
    ConnectionDriver.update(query, {}, update, {upsert: true, new: true}, processDbResult);
  } else {
    ConnectionDriver.insertOne(props, processDbResult);    // NF updated 22/04/2015
  }

  function processDbResult(err, result) {
    // runs the job immediately console.log('err', err);
    // runs the job immediately console.log('result', result);
    ...
    if (err) {
      ...
      var res = result;     // result is different for findAndModify() vs. insertOne(). NF 20/04/2015
      if ( res ) { // very odd check sense original functions returned only a number....
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  ConnectionDriver.findAndLockNextJob(jobName, definition, this._nextScanAt, createJob, this, cb);
};
...
Agenda.prototype._unlockJobs = function(done) {
  ...
  ConnectionDriver.updateMany({_id: { $in: jobIds } }, { set: { lockedAt: null } }, done);    // NF refactored .update() 22/04/2015
};
...
function processJobs(extraJob) {
  ...
  function lockOnTheFly() {
    ...
    var criteria = {
      ...
      disabled: { $ne: true } // we be removed inside the sql driver
    };
    ...
    ConnectionDriver.update(criteria, sort, update, options, function(err, resp) {
      if (resp) {
        var job = createJob(self, resp);
...
/**
 * used to decide whether to run based on master setting 
 * by default setting entered to Agenda all will run
 * @param onlyMaster
 * @param isMaster
 * @private
 */
function _runAllowed(onlyMaster, isMaster) {
  if(onlyMaster) {
    if(isMaster) {
      return true;    
    } 
